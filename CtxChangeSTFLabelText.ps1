<#
.SYNOPSIS
	Change enties in the ExplicitFormsCommon.XX.resx files
.DESCRIPTION
	Change enties in the ExplicitFormsCommon.XX.resx files
.PARAMETER DataField
	Select the datafield you want to change the value for
	- DomainUserAssistiveText -> "domain\user or user@domain.com" (EN)
	- DomainUserLabel -> "Domain\User:" (EN)
	- UsernameLabel -> "User name:" (EN)
	- PasswordLabel -> "Password:" (EN)
.PARAMETER Value
...
param(
		...
		[ValidateSet("DomainUserAssistiveText", "UsernameLabel", "PasswordLabel", "DomainUserLabel", IgnoreCase = $false)]
		[String]$DataField = "DomainUserAssistiveText",
	...
	ForEach ($File in $Files) {
		...
		if($Restore) {
			...
			try {
				...
				$xml = New-Object XML
				$xml.Load($xmlfile)
				[string]$xpath="//root/data[@name='{0}']/value" -f $DataField
				$element = $xml.SelectSingleNode($xpath)
				Write-Host -NoNewLine "Text: `"$($element.InnerText)`" into `"$($Value)`" "
